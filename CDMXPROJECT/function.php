...
$result =mysqli_query($this->dbh,"SELECT email FROM users WHERE email='$email' lIMIT 1");
return $result;
}

 //for Department availblty
 public function Departmentavailblty($dname) {
	$result =mysqli_query($this->dbh,"SELECT Department FROM department WHERE Department='$dname' lIMIT 1");
	return $result;
	}
...
public function totalDesignationRowCount(){
 ...
 public function Designationavailblty($dname) {
	$result =mysqli_query($this->dbh,"SELECT designation FROM designation WHERE designation='$dname' lIMIT 1");
	return $result;
	}
